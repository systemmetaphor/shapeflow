...
namespace ShapeFlow.Loaders.KriativityReflectedModel
{
    public static class ModelSorter
    {
            ...
            where T : ReflectedObject
        {
            ...
            var allTypesSortedNodes = sortedNodes.Where(root => values.Any(v => v.DotNetType.FullName == root)).ToList();

            var vertList = graph.Vertices.ToDictionary(s => s,
                s =>
                {
                    IEnumerable<Edge<string>> outEdges = null;
                    graph.TryGetOutEdges(s, out outEdges);
                    return outEdges;
                });

            var nodesWithoutParents = allTypesSortedNodes.Where(node => vertList[node] == null || !vertList[node].Any()).ToList();
            allTypesSortedNodes.RemoveAll(s => nodesWithoutParents.Contains(s));
            allTypesSortedNodes.AddRange(nodesWithoutParents);

            var notAnalyzedTypes = values.Where(s => !allTypesSortedNodes.Contains(s.DotNetType.FullName)).Select(t => t.DotNetType.FullName).ToList();
            ...
            {
                results.Add(values.First(v => v.DotNetType.FullName == sorted));
            }

            return results;
        }

        private static void AddTypeDependencies(IMutableVertexAndEdgeListGraph<string, Edge<string>> graph, Type type)
        {
